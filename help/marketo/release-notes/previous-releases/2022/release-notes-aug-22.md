...
* **[Aktivera/inaktivera alla publicerade dialogrutor samtidigt för Dynamic Chat](/help/marketo/product-docs/demand-generation/dynamic-chat/automated-chat/dialogue-overview.md#disable-enable-all-dialogues){target="_blank"}**: Aktivera/inaktivera alla publicerade dialogrutor på konfigurationssidan globalt med en knapptryckning.

* **[Anpassade avatarer för Dynamic Chat](/help/marketo/product-docs/demand-generation/dynamic-chat/setup-and-configuration/configuration.md#agent-settings){target="_blank"}**: Ladda upp en anpassad chatbot-avatar så att den kan anpassas efter ert varumärke.
...
* **Visa mapphierarkin för målmappen i dialogrutan Flytta**: Om du visar mapphierarkin för varje mapp blir det enklare att flytta resurser, vilket minskar sannolikheten för att de placeras i fel mapp.

* **[Uppdaterade skärmar i nästa generation](/help/marketo/product-docs/marketo-engage-modern-ux/toggle-switch.md){target="_blank"}**: Vi levererar nya, uppdaterade skärmar i nästa generation av upplevelser som ger en uppdaterad design- och användarvänlighetsförbättring som är tillgänglig via växlingsknapp:
...
* **Synlighet för anpassad webbkrok-huvud**: Anpassade Webkrohuvuden visas nu på fliken Admin > Webhooks för bättre synlighet.
...
* **Behörighet att godkänna formulär**: Ny behörighet för att styra vilka formgivare som kan godkänna ändringar i ett formulär i linje med andra Design Studio-resurser. Detta förhindrar andra designers från att göra ändringar i formulären utan att någon annan behöver ge sitt godkännande.

* **Utför alltid kampanjåterspelning efter anonym sammanslagning**: Anonym lead-sammanslagning inträffar innan kampanjen spelas upp så att anpassade fältfilter fungerar tillförlitligt när anonym kampanjåterspelning görs.

## Marknadsföringsdatamiljö {#marketing-data-environment}

* **Korrigera användargränssnittstrunkering av fält för anpassat objekt som används av**: Det är nu enklare att identifiera anpassade objektfält som används så att du kan ta bort fält från ett anpassat objekt när det behövs.
