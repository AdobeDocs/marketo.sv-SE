...
* **Redigerare för mall för landningssida**: Landningssidmallar fungerar nu som andra Design Studio-resurser.
...
* **Söka efter bilder och filer**: Hitta snabbt dina överförda resurser.
* **Ursprunglig källinformation i Salesforce**: Uppdatera Marketo AppExchange-appen för nya fält.
* **Inkommande information för alla leads**: Alla leads har nu information om företag och platser.
...
* **Ursprunglig hänvisare, fråga och sökmotor**: Den här informationen sparas för alla leads som kommer till dina Munchkin-aktiverade sidor och landningssidor.
* **Skräppostkontroll**: Du bör se en dramatisk minskning av antalet skräppostleads på grund av ändringar i våra formulär.
* **Inlämning av ärendet online**: Behöver du hjälp? Skicka din begäran via vår webbplats för lyckade åtgärder.
...
* **Salesforce-länk i Skicka aviseringsinformation**: När du använder en skicka-aviseringsinformationstoken i ett e-postmeddelande och leadet finns i Salesforce, får du länkar till Salesforce och Marketo.
...
* **Datum för senaste uppdatering vid sammanslagning**: När du sammanfogar leads ser du nu när leadet senast uppdaterades.
...
* **Förbättrad e-posttestsändning**: Du kan nu välja att skicka textversionen av e-postmeddelandet när du skickar ett testmeddelande. Dessutom kommer Marketo att lagra de 10 senaste testversionerna av e-postmottagare som du använde för att snabba upp e-posttestningen.
* **Smarta ikoner för smarta kampanjer**: Ikonerna i trädet Smart Campaign visar nu om kampanjen är aktiv eller inte.
* **Konvertera nummerfält till bakgrundsmusik**: Har du ett anpassat heltalsfält som du vill behandla som ett poängvärde i stället? Konvertera det till en poäng i avsnittet Fälthantering på Admin-sidan.
...
* **API**: Nu kan du interagera direkt med Marketo.
* **Outlook-plugin**: Tillåt säljarna att skicka e-post från Outlook och spåra skickade/klickade/öppnade meddelanden i Marketo. Till och med avfyra aktivitet!
...
* **Kontroll av pausad Blockeringslista och marknadsföring**: Nu kan ni styra e-postflödena baserat på vilka kriterier ni vill.
...
* **Exportera lead i bakgrunden**: Nu finns det ingen gräns för hur många namn du kan exportera och webbläsaren är inte längre spärrad. Dessutom kan du se exportens status visuellt och när den är klar.
...
* **Genom en populär begäran lade vi till nya e-postkolumner i analysavsnittet**: E-postdiagrammet visar nu fler detaljer, inklusive antal skickade, studsade, leveransprocent och att prenumerationen har avbrutits. Kampanjen **Fliken Sammanfattning visar inte längre sammanfattningen av e-postprestanda**: Det finns nu en e-postflik om du vill visa den här informationen. Den här rapporten är också tillgänglig per kampanj.
...
* En ny **Knappen&quot;Markera alla&quot;** finns det nu så att du kan arbeta med en hel lista samtidigt. Använd den här nya funktionen med de nya Lead Actions. Klicka på Lead Actions överst i listan så ser du ett antal nya kraftfulla funktioner. Detta inkluderar alla vanliga flödessteg.
...
* **Nya kolumner för säljprojektsdata (optisk)**: Detta ger svar på viktiga frågor om marknadsföringsaktiviteternas effektivitet.
...
* **Sammanfoga och hitta dubbletter**: Med filtret Sök dubbletter kan du söka efter dubbletter baserat på vilken fältmatchning som helst. När du har hittat dubbletterna markerar du en eller flera och trycker sedan på knappen &quot;sammanfoga&quot;.
...
* **Överföra aktiviteter från Marketo till Salesforce**: Klicka bara på admin->Salesforce och kontrollera de aktiviteter du vill dela med ditt säljteam.
...
* **medlemskapstrendflik på samma skärm**: Det bästa sättet är att ha vissa fokuserade kampanjer som representerar mål som ni försöker uppnå. Ett mål kan vara när du lämnar ett tips till säljteamet. I det här diagrammet visas den specifika trenden per månad. Du kan till och med visa helskärmsläge eller exportera data till Excel!
* **Exportera från alla** lista, smart lista och till och med aktiviteten resulterar i en kampanj. Titta på rutnätets nederkant och klicka på exportera.
...
* **Ökad serverproduktion med 300 %** med klustring och uppgraderingar av företagsmaskinvara. Vi arbetar för utmärkt drifttid och serverprestanda. Vi har ännu fler överraskningar på den här punkten.
...
* **Ny informationssida för lead**: Formateringen är inte klar än, men funktionaliteten är bra. Du kan öppna flera leads nu i ett separat fönster och hålla reda på dem medan du går någon annanstans i programmet. Det här är samma popup-fönster som används för att länka från den populära webbaktivitetsrapporten. Prestanda och laddningstid har förbättrats med över 250 %.
...
* **Redigera standardtext för att avbryta prenumeration**: Klicka på admin och sedan på e-post så kan du redigera den avbrutna texten som skrivs ut längst ned i e-postmeddelanden. HTML- och textversioner för e-post med flera delar.
...
* **Hastighetsförbättringar** på munchkin-skript och flera kategorier med smarta listor och sökningar.
* **Dra och släpp** fungerar nu även om du tar ikonen. Jag vet, det här var irriterande. Alla lagade!
* **Fler värdbaserade jQuery-plugin-program** för användning på landningssidor. Alla typer av dynamiska funktioner finns på landningssidor med skräddarsydd HTML. Be oss om hjälp idag. Vi kommer att publicera Instruktioner snart. Vissa möjligheter:
...
* **Uppdateringar om Utforskarträd**: Försök att högerklicka på något av objekten i trädet. Du kan använda menyn Gå till för att hoppa direkt till en flik.
* **Program inom marknadsföringsverksamhet**: Skapa kapslade program för att ordna era kampanjer. Håll era smarta kampanjer små genom att undvika upprepade flödessteg.
* **Ta bort och byt namn** fungerar för mappar, program i trädet.
...
* **Större prestandaoptimeringar** runt omkring.
...
* **Förbättringar i kolumnväljaranvändargränssnittet**: Välj kolumner för ett rutnät genom att klicka på utlösaren i sidhuvudet.
* **Inställningar för Sticky Column**: När du sorterar eller ändrar ordning på kolumner sparas inställningarna i Marketo.
* **Organisationsverktyg i lead-databas**:
