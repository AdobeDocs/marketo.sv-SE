...
description: Anslut din [!DNL BrightTALK] kanal till Marketo via LaunchPoint med autentiseringsuppgifter som endast är API-baserade.
...
1. Inloggningsskärmen visas med en fråga: Klient-ID, Klienthemlighet, Identitetstjänstens URL och URL för återställningstjänst. Logga in på Marketo för att få den här informationen.

## Steg i Marketo {#steps-in-marketo}

>[!NOTE]
>
>Du måste nu konfigurera [!DNL API Only User Role] och [!DNL API User] för att kunna begränsa vilka behörigheter [!DNL BrightTALK] har i din Marketo-instans. Artiklar är tillgängliga för dessa steg.
...
1. Ange ett **[!UICONTROL Display Name]** som du vill använda. Klicka på listrutan **[!UICONTROL Service]** och välj **[!UICONTROL Custom]** (markera _inte_ [!DNL BrightTALK]).
   ...
   >[!CAUTION]
   >
   >Kom ihåg att inte markera [!DNL BrightTALK] i listrutan. Det här fältet håller på att tas bort och om du väljer det kan det skapa betydande problem med din [!DNL Marketo/BrightTALK]-integrering.
...
När autentiseringsuppgifterna har autentiserats har du officiellt anslutit [!DNL BrightTALK] till Marketo. Nästa steg är att avgöra vilka datafält du vill synkronisera. Om du behöver hjälp med det kontaktar du [BrightTALK Support](https://www.brighttalk.com/){target="_blank"}.
